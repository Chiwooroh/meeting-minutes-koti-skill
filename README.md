...
한국어 회의 **녹음 파일**과 **회의공지 메일(.eml)** 을 입력받아, 공무원 보고형식의 **회의록**을 작성하고 공식 한글 **HWPX 파일**까지 자동으로 생성하는 [Claude Code](https://code.claude.com/docs/en/skills) 스킬(Agent Skill)입니다.
...
5. [호출 방법](#호출-방법)
6. [빠른 시작](#빠른-시작)
7. [전체 워크플로우 (단계별)](#전체-워크플로우-단계별)
8. [스크립트 레퍼런스](#스크립트-레퍼런스)
9. [최종 산출물 규칙](#최종-산출물-규칙)
10. [작성 기준 (문체·내용 선별)](#작성-기준-문체내용-선별)
11. [HWPX 서식 규칙](#hwpx-서식-규칙)
12. [폴더 구조](#폴더-구조)
13. [문제 해결 (Troubleshooting)](#문제-해결-troubleshooting)
14. [자주 묻는 질문](#자주-묻는-질문)
15. [개인정보·보안 주의](#개인정보보안-주의)

---

## 무엇을 하는 스킬인가요

연구실·공공기관에서 회의 녹음을 받아 **사람이 직접 회의록을 타이핑하던 작업**을 자동화합니다. Claude Code가 작업 폴더의 녹음·메일을 인식하고, 아래 과정을 대신 수행합니다.
...
Claude Code의 스킬은 `SKILL.md`가 든 폴더를 **스킬 디렉터리**에 두면 됩니다.

- 개인(사용자) 스킬: `~/.claude/skills/<스킬명>/` — Windows는 `C:\Users\<사용자>\.claude\skills\<스킬명>\`
- 프로젝트 스킬: 프로젝트 루트의 `.claude/skills/<스킬명>/`
...
2. `meeting-minutes-koti` 폴더를 개인 스킬 폴더로 복사합니다.

   ```powershell
   Copy-Item -LiteralPath ".\meeting-minutes-koti" `
     -Destination "$env:USERPROFILE\.claude\skills\meeting-minutes-koti" -Recurse -Force
   ```

3. Claude Code를 다시 시작하거나 새 세션을 엽니다. 설치되면 `/meeting-minutes-koti`로 목록에 나타납니다.

> 프로젝트 단위로만 쓰려면 `meeting-minutes-koti` 폴더를 해당 프로젝트의 `.claude/skills/`에 두어도 됩니다.

---

## 호출 방법

Claude Code 스킬은 두 가지 방식으로 실행됩니다.

| 방식 | 사용법 | 설명 |
| --- | --- | --- |
| **자동 호출** | 그냥 작업을 요청 | Claude가 `SKILL.md`의 `description`을 보고 회의록 작업이라고 판단되면 스스로 이 스킬을 적용합니다. |
| **명시적 호출** | `/meeting-minutes-koti` | 사용자가 직접 스킬을 지정해서 실행합니다. |

> Codex의 `$meeting-minutes-koti` 트리거와 달리, Claude Code는 `/스킬명` 슬래시 명령을 쓰거나 자연어로 요청하면 자동으로 스킬을 선택합니다.

---

## 빠른 시작

회의 녹음(과 선택적으로 `.eml`)이 들어 있는 폴더에서 Claude Code에 다음과 같이 요청합니다.

```text
이 폴더 녹음 파일들로 파일당 회의록 1개씩 작성해줘
```
...
> 아래 예시는 작업 폴더에서 직접 스크립트를 실행하는 방법입니다. 보통은 Claude가 자동으로 호출하지만, 수동 실행·디버깅에 참고하세요. 경로는 설치 위치(`$env:USERPROFILE\.claude\skills\meeting-minutes-koti`)에 맞춰 조정합니다.
...
├─ README.md                  # 이 사용설명서
├─ README_INSTALL.md          # 설치 요약
└─ meeting-minutes-koti/      # ← Claude Code 스킬 폴더(~/.claude/skills/)로 복사하는 스킬 본체
   ├─ SKILL.md                # 스킬 정의·워크플로우 (Claude가 읽음)
   ├─ assets/
...
| 증상 | 원인 / 해결 |
| --- | --- |
| `/meeting-minutes-koti`가 목록에 안 보임 | `meeting-minutes-koti` 폴더가 `~/.claude/skills/`(또는 프로젝트 `.claude/skills/`)에 있는지, 폴더 안에 `SKILL.md`가 있는지 확인 후 세션 재시작 |
...
**Q. Codex에서도 쓸 수 있나요?**
A. 이 저장소는 Claude Code용입니다. 동일한 `SKILL.md`/스크립트 구조라 Codex에서도 쓸 수 있지만, 설치 경로를 `~/.codex/skills/`로 바꾸고 `$meeting-minutes-koti` 트리거로 호출하면 됩니다.
...
> 이 저장소는 회의록 자동화 **Claude Code 스킬**과 그 사용설명서를 담고 있습니다. 스킬 본체는 `meeting-minutes-koti/` 폴더입니다.
